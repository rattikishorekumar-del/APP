...
// NOTE: "Know Your Status" used to call a separate Apps Script deployment (STATUS_WEB_APP_URL).
// That's been removed — see searchStatus() below, which now calls the secure 'getStatus' action
// on this same WEB_APP_URL instead. The old approach (and the separate script) returned the
// ENTIRE Selections sheet to the browser for every search; this one asks the server for a single
// passport number and gets back only that one matching record. See the compliance audit for details.
...
let session = { role: null, name: null, trade: null, state: null, experience: null, officeName: null, phoneNumbers: [] };
...
async function apiGet(action, params={}, baseUrl=WEB_APP_URL){
  if (!baseUrl || baseUrl.indexOf('PASTE_YOUR') === 0) throw new Error('App is not configured yet — set WEB_APP_URL in app.js');
...
function doLogin(){
  ...
  apiPost('login', { loginId, password: pass }).then(res => {
    hd();
    session = {
      role: res.role, name: res.name,
      trade: res.trade || '', state: res.state || '', experience: res.experience || '',
      officeName: res.officeName || '', phoneNumbers: res.phoneNumbers || []
    };
...
function doLogout(){
  session = { role: null, name: null, trade: null, state: null, experience: null, officeName: null, phoneNumbers: [] };
...
async function searchStatus(){
  ...
  ld('Searching…');
  try {
    const r = await apiGet('getStatus', { passport: raw });
    hd();
    renderStatus(r.data);
  } catch (err) {
    hd();
    if (/no record found/i.test(err.message)) showStatusErr('No record found for passport "' + raw + '". Please check and try again.');
    else showStatusErr('Could not connect. Please try again. (' + err.message + ')');
  }
}
