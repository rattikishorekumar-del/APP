  ...
  grid.innerHTML = jobsCache.map(job => {
    ...
    return `<div class="job-card">
      <div class="job-img-wrap">
        <img src="${escapeHtml(job.imageUrl)}" alt="${escapeHtml(job.country)}" loading="lazy"/>
...
// Note: install-banner dismissal is kept in memory only (no localStorage), so the banner
// may reappear on a fresh page load — that's an intentional trade-off, see setup guide.
let pwaDismissed = false;
function setupPWA(){
  const iconSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="512" height="512" viewBox="0 0 512 512"><rect width="512" height="512" rx="80" fill="#0049B7"/><text x="256" y="340" font-size="260" text-anchor="middle" font-family="Apple Color Emoji,Segoe UI Emoji,sans-serif">🌍</text></svg>`;
  const iconUrl = 'data:image/svg+xml;base64,' + btoa(iconSvg);
  document.getElementById('pwa-icon').href = iconUrl;
 
  const manifest = {
    name: 'Abroad Jobs', short_name: 'Abroad Jobs', description: 'Abroad Jobs Portal',
    start_url: './', display: 'standalone', orientation: 'portrait',
    background_color: '#0049B7', theme_color: '#0049B7',
    icons: [
