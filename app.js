...
// ═══════════════ USER INFO RIBBON (shown below the header once logged in) ═══════════════
function uibItem(icon, label, val, truncate){
  if (!val) return '';
  const safe = escapeHtml(val);
  const attrs = truncate ? ` style="max-width:230px;overflow:hidden;text-overflow:ellipsis;" title="${safe}"` : '';
  return `<span class="uib-item"${attrs}>${icon} <b>${label}:</b> ${safe}</span>`;
}
function buildUserInfoBar(){
  const bar = document.getElementById('user-info-bar');
  const contact = (session.phoneNumbers || []).join(', ');
  let html = '';
  if (session.role === 'candidate') {
    html = [
      uibItem('🎯','Trade', session.trade),
      uibItem('📍','State', session.state),
      uibItem('🕒','Experience', session.experience),
      uibItem('📞','Contact', contact)
    ].join('');
  } else if (session.role === 'associate') {
    html = [
      uibItem('🏢','Office', session.officeName, true),
      uibItem('📍','State', session.state),
      uibItem('📞','Contact', contact)
    ].join('');
  }
